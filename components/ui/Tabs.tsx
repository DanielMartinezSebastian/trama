  ...
  return (
    <div className={`ui-tabs ${vcls(variant)} ${className}`}>
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === "ArrowRight") setI((cur + 1) % tabs.length);
          if (e.key === "ArrowLeft") setI((cur - 1 + tabs.length) % tabs.length);
        }}
      >
        {tabs.map((t, k) => (
          <button key={t + k} role="tab" aria-selected={k === cur} tabIndex={k === cur ? 0 : -1} className="ui-tabs__tab" onClick={() => setI(k)}>
            {t}
          </button>
        ))}
      </div>
